...
int main(int artc, char** argv) {
	Mat src1 = imread("F:/opencv_tutorial-master/images/LinuxLogo.jpg");
	Mat src2 = imread("F:/opencv_tutorial-master/images/WindowsLogo.jpg");
	...
	Mat add_result = Mat::zeros(src1.size(), src1.type());	//¼Ó
	add(src1, src2, add_result);
	imshow("add_result", add_result);

	Mat sub_result = Mat::zeros(src1.size(), src1.type());	//¼õ
	subtract(src1, src2, sub_result);
	imshow("sub_result", sub_result);

	Mat mul_result = Mat::zeros(src1.size(), src1.type()); //³Ë
	multiply(src1, src2, mul_result);
	imshow("mul_result", mul_result);

	Mat div_result = Mat::zeros(src1.size(), src1.type());//³ý
